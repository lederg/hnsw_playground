 ...
 public:
    ...
    Index(const std::string &space_name, const int dim) : space_name(space_name), dim(dim) {
        ...
        if (space_name == "l2") {
            ...
            l2space = new hnswlib::InnerProductSpace(dim);
            normalize = true;
        } else if (space_name == "multivector") {
            l2space = new hnswlib::MultiVectorL2Space<unsigned int>(dim);
            l2space_multivector = static_cast<hnswlib::MultiVectorL2Space<unsigned int>*>(l2space);
            is_multivector = true;
        } else {
    ...
    void addItems(py::object input, py::object ids_ = py::none(), int num_threads = -1, bool replace_deleted = false, py::object docids_ = py::none()) {
        ...
        std::vector<size_t> docids = get_input_ids_and_check_shapes(docids_, rows);
        if (docids.size()) {
            if (!is_multivector) {
                throw std::runtime_error("docids are only supported for multivector space");
            }
            if (docids.size() != rows) {
